...
//EN DESORDEN ->  Algoritmo: Busqueda con dos indices. (en el peor de los casos el time complexity serà de O(n/2))
...
No se debe confundir con 'conversión', dado que esta ultima se hace de forma explicita es decir el programador es quien indicarà a JS que datos se van a convertir y en que.*/
const x = "2"; //string
const y = 5; //int
...
//Si queremos que para este ultimo caso se realice una suma y no una concatenaciòn debemos hacer una converseión (para evitar que JS coersa los datos):
...
es importante dado que ayuda a mantener un codigo limpio  y reducir el uso de declaraciones de variables
Para ejecutarlo se envuelve el codigo en parentesis y luego de esto se abre y cierra otro par de parentesis (function() */
...
La combinacion de los componentes es lo que finalmente entregará la interfaz de usuario del aplicativo.*/

//10.	¿Qué es un servicio en Angular? ¿En qué se diferencia de un Pipe?
/* Un servicio es un codigo reusable que estarà disponible para todos los componentes de la aplicación.
Su uso mas comun es el de acceso o manipulación de información, por ejemplo de la base de datos, y si bien esto podria hacerse desde el componente, sería una mala practica e iria en contra de la mision del componente el cual es unicamente presentar informacion al usuario.
Por lo anterior las tareas como establecer la comunicación con el backend debe hacerse a traveés de  servicios */
...
Estos ciclos de vida nos permiten correr un codigo en un momento especifico del ciclo de vida de un compoente o directiva
...
Por ejemplo, todos los que se encuentre dentro de OnInit de un componente "x". se ejecutarà cuando se renderize el comopente "x"*/
...
/* Angluar realiza su renderizado en el front en funcion de acciones que se realicen desde aquì todo esto para garantizar una mejor experiencia al usuario desde que angular es un SPA
Renderizarse en el front le brinda la ventaja de poder ser una Single Page Application pero esto puede tener una desventaja dado que al no estar compilada la información desde el lado del servidor para los buscadores como Google la pagina no será visible 
Sin embargo tenemos una tecnologìa llamda Angular Universal que harà lo que se llama un Server Side Rendering o un renderizado desde el servidor garantizando que nuestra pagina WEB sea visible para el SEO */
